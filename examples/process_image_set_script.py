...
image_path = r'G:\My Drive\Terminus\InfraTracker\Imaging Trips\2-22-25_Trip\BestSettings' # full path to your images
...
if darkframe_file_path is not None:
    if not os.path.exists(darkframe_file_path):
        ...
        print("unable to find provided darkframe file, proceeding without one...", file=sys.stdout)
    else:    print("darkframe file: " + darkframe_file_path)
else:    print("no darkframe file provided, proceeding without one...", file=sys.stdout)
...
for image_filename in image_names:
    ...
    print(image_filename, file=sys.stdout)
...
print("data saved to: " +filename, file=sys.stdout)
